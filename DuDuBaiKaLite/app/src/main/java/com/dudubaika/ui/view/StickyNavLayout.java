...
public class StickyNavLayout extends LinearLayout implements NestedScrollingParent
{
    ...
    //从fragment里面Recycleview传递过来的值 是否可以向下滑动
    ...
    public void onNestedPreScroll(View target, int dx, int dy, int[] consumed)
    {
        ...
        boolean hiddenTop = dy > 0 && getScrollY() < mTopViewHeight;//上滑
//        boolean showTop = dy < 0 && getScrollY() >= 0 && !ViewCompat.canScrollVertically(target, -1);
        boolean showTop = dy < 0 && getScrollY() >= 0 && !isdown;//下滑
    ...
    public boolean onTouchEvent(MotionEvent event)
    {
        ...
        switch (action)
        {
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                LogUtil.i("StickyNavLayout","mLastY="+mLastY+"dy="+dy);
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh)
    {
        ...
        LogUtil.i("StickyNavLayout","onSizeChanged h="+h);
    }
    ...
    public void scrollTo(int x, int y)
    {
        ...
        if (y < 0)
        {
           ...
           EventBus.getDefault().postSticky(new IsRefreshUsedEvent(true));
            LogUtil.i("StickyNavLayout","刷新可用 ");
        }
        if (y > mTopViewHeight)
        {
            y = mTopViewHeight;
            EventBus.getDefault().postSticky(new IsRefreshUsedEvent(false));
            LogUtil.i("StickyNavLayout","刷新不可用 ");
        }
        if (y != getScrollY())
        {
            //刷新不可用
        ...
        currentY = y;
        LogUtil.i("StickyNavLayout","scrollTo y="+y);
    }
    ...
    public boolean onInterceptTouchEvent(MotionEvent ev) {
        ...
        switch (action) {
            ...
            case MotionEvent.ACTION_MOVE:
                float dy = y -downY;
                if (currentY==mTopViewHeight && dy>0){
//                    getChildAt(0).
                    return false;
                }
                LogUtil.i("StickyNavLayout", "mLastY=" + mLastY + "dy=" + dy);
    ...
    @Subscribe
    public void getMsgTest(HomeFragmentIsDown event){
        isdown = event.isDown();
    }
