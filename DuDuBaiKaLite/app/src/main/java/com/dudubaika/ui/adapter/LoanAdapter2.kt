...
class LoanAdapter2(var context: Context, private var list:List<MineLoanBean.PaynoteListBean>, resId:Int): CommonAdapter<MineLoanBean.PaynoteListBean>(context,list,resId) {
    ...
    override fun convert(holder: ViewHolder?, item: MineLoanBean.PaynoteListBean?, posotion: Int) {
        ...
        if (!UserUtil.getIsfirst(context)){
            if (posotion==0){
        ...
        UserUtil.saveIsfirst(context,true)
        ...
        holder!!.setOnClickListener(R.id.btnred, {
            ...
            TCAgent.onEvent(mContext, TalkingDataParams.CHANGE_NOTE_SATTUS, "waitrepment")
        })
        holder!!.setOnClickListener(R.id.btnDelete, {
            ...
            mDatas.removeAt(posotion)
            notifyDataSetChanged()
            TCAgent.onEvent(mContext, TalkingDataParams.CHANGE_NOTE_SATTUS, "repaid")
        })
