...
class RapidFragment @SuppressLint("ValidFragment")
    ...
    override fun showError(url: String, msg: String) {
        if (ApiSettings.GET_BANNER_LIST == url || ApiSettings.GET_PRODUCT_LIST == url) {
            ...
            progress.ll_error.setOnClickListener {
                mPresenter.getProductInfo("2",index.toString(),pageSize)
            }
        }
    }
...
//        hotProductAdapter = initHotProductAdapter()
        hotProductAdapter = ProductAdapter(mContext!!,totalList!!,product_type,GlobalParams.FALG_THREE).initHotProductAdapter()
