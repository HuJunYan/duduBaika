 ...
 * 筛选界面数据
 */
class FindListAdapter(data: List<UsersAuthLimitBean.ProductListBean>?,   var context: Context,var tag:String) : BaseQuickAdapter<UsersAuthLimitBean.ProductListBean, BaseViewHolder>(R.layout.item_hot_product2, data) {


    override fun convert(holder: BaseViewHolder, item: UsersAuthLimitBean.ProductListBean) {
        ...
        if (!TextUtils.isEmpty(item.activity_name)) {
            ...
            holder.getView<TextView>(R.id.hot).visibility = View.VISIBLE
        } else {
